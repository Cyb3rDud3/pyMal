...
def getRegistryKey(key_name, registry_path, HKLM=False):
    """
    :param key_name: registry key name
    :type key_name: str
    :param registry_path: full path to registry key
...
def extract_zip(extraction_path, file_to_extract):
    """
...
def install_with_pyinstaller(pyinstaller_path, base_dir, file_to_install, onefile=True, hidden_imports=[]):
    """
    ...
    distpath = base_dir
    to_run = "{} {} {} {} --distpath {} --icon=NONE".format(pyinstaller_path, onefile_str, hidden_imports_list,
                                                            os.path.join(base_dir, file_to_install), distpath)
...
def run_detached_process(code, is_powershell=False):
...
def base85_encode_file(file_path):
    """
